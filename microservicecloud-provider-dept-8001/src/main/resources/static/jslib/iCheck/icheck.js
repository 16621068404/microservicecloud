...
(function(win, doc, $) {
    ...
    if (!win.ichecked) {
        win.ichecked = function() {
            ...
            var pointer = win.PointerEvent ? [ 'pointer', mouse[1], mouse[2], mouse[3], mouse[4] ]
                    : [ 'MSPointer', 'Down', 'Up', 'Over', 'Out' ];
            var touch = [ 'touch', 'start', 'end' ];
            var noMouse = (isTouch && isMobile) || isPointer;

            // choose events
            var hoverStart = noMouse ? (isTouch ? touch[0] + touch[1] : pointer[0] + pointer[3])
                    : mouse[0] + mouse[3];
            var hoverEnd = noMouse ? (isTouch ? touch[0] + touch[2] : pointer[0] + pointer[4])
                    : mouse[0] + mouse[4];
            var tapStart = noMouse ? (isTouch ? false : pointer[0] + pointer[1]) : mouse[0]
                    + mouse[1];
            ...
            var styleTag;
            var styleList;
            var styleArea = defaults.areaStyle !== false ? 'position:absolute;display:block;content:"";top:#;bottom:#;left:#;right:#;'
                    : 0;
            ...
            var style = function(rules, selector, area) {
                ...
                if (!selector) {
                    selector = 'div.'
                            + (area ? areaClass + area + ':after' : divClass + ' input.'
                                    + nodeClass);
                }
            ...
            // class toggler
            var toggle = function(node, className, status, currentClass, updatedClass, addClass,
                    removeClass) {
            ...
            var process = function(data, options, ajax, silent) {
                ...
                while (element--) {
                    ...
                    for (nodeDataProperty in nodeData) {
                        ...
                        nodeDataCache[nodeDataProperty.replace(/checkbox|radio|class|id|label/g,
                            ...
                            while (nodeInheritLength--) {
                                ...
                                if (nodeAttrCache[nodeInheritItem] !== undefined) {
                                    if (nodeInheritItem == 'class') {
                                        ...
                                        styler.setAttribute(nodeInheritItem,
                                                nodeInheritItem == 'id' ? baseClass + '-'
            ...
            var operate = function(node, parent, key, method, silent, before, ajax) {
                ...
                if ((!before || ajax) && method !== 'click') {
                    ...
                    states.indeterminate = [
                            node.getAttribute('indeterminate') == 'true' || !!node.indeterminate,
            ...
            // state changer
            var change = function(node, parent, states, changes, key, settings, method, silent,
                    before, ajax, loop) {
                var type = node.type;
                ...
                if (parent) {
                    ...
                    for (property in changes) {
                        ...
                        if (states[property][0] !== value && method !== 'updated'
                                && method !== 'click') {
                            node[property] = value;
                        }
                        ...
                        if (settings[property] !== value) {
                            ...
                            if (property == 'checked') {
                                ...
                                if (!loop && value && (!!hashes[key].done || ajax)
                                        && type == 'radio' && !!node.name) {
                                    ...
                                    while (radiosLength--) {
                                        ...
                                        if (node !== radio && hashes[radioKey]
                                            ...
                                            change(radio, false, radioStates, radioChanges,
                                                    radioKey, hashes[radioKey], 'updated', silent,
                                                    before, ajax, true);
                                        }
                                    }
                                }
                            ...
                            // cache classes
                            classes = [ settings[property + 'Class'], // 0, example: checkedClass
                            settings[property + typeCapital + 'Class'], // 1, example:
                                                                        // checkedCheckboxClass
                            settings[states[property][1] + 'Class'], // 2, example:
                                                                        // uncheckedClass
                            settings[states[property][1] + typeCapital + 'Class'], // 3, example:
            ...
            $.fn.icheck = function(options, fire) {
                ...
                // detect methods
                if (/^(checked|unchecked|indeterminate|determinate|disabled|enabled|updated|toggle|destroy|data|styler)$/
                        .test(options)) {
                    ...
                    .on(
                            ...
                            function(event) {
                                ...
                                if (key) {
                                    ...
                                    var activate;
                                    var states = [
                                            [ 'label', settings.activeLabelClass,
                                                    settings.hoverLabelClass ],
                                    ...
                                    if (emitter == tapStart || emitter == tapEnd) {
                                        ...
                                        if (!!settings.mirror && !!states[1][1]) {
                                            ...
                                            while (partner.length--) {
                                                toggle(partner[partner.length], states[1][1],
                                                        emitter == tapStart ? 1 : 0);
                                            }
                                        }

                                        // fast click
                                        if (div && emitter == tapEnd && !!settings.tap && isMobile
                                                && isPointer && !operaMini) {
                                            activate = true;
                                        }
                                        ...
                                        if (!!states[0][2]) {
                                            toggle(self, states[0][2], emitter == hoverStart ? 1
                                                    : 0);
                                        }
                                        ...
                                        if (!!settings.mirror && !!states[1][2]) {
                                            ...
                                            while (partner.length--) {
                                                toggle(partner[partner.length], states[1][2],
                                                        emitter == hoverStart ? 1 : 0);
                                            }
                                        }

                                        // fast click
                                        if (div && emitter == hoverEnd && !!settings.tap
                                                && isMobile && isTouch && !operaMini) {
                                            activate = true;
                                        }

                                        // click
                                    } else if (div) {
                                        if (!(isMobile && (isTouch || isPointer))
                                                || !!!settings.tap || operaMini) {
                                            activate = true;
                                        }
                                    }
                                        ...
                                        setTimeout(function() {
                                            ...
                                            if (target.tagName !== 'LABEL') {
                                                if (!settings.change
                                                    ...
                                                    input = $(self).find('input.' + className)
                                                            .click();
                                ...
                                if (key) {
                                    ...
                                    if (emitter == 'click') {
                                        ...
                                        if (parent && !self.disabled) {
                                            operate(self, parent, key, 'click'); // 'click'
                                                                                    // method
                                        }
                                        ...
                                        if (!!settings.mirror && !!states[1]) {
                                            ...
                                            while (label.length--) {
                                                toggle(label[label.length], states[1],
                                                        emitter == 'focusin' ? 1 : 0);
                                            }
                                        }

                                        // keyup or keydown (event fired before state is changed,
                                        // except Opera 9-12)
                                        ...
                                        if (emitter == 'keyup') {
                                            ...
                                            if (self.type == 'checkbox' && event.keyCode == 32
                                                    && settings.keydown || self.type == 'radio'
                                                    && !self.checked) {
                                                ...
                                                converters : {
                                                    'text html' : function(data) {
                                                        if (win.icheck.autoAjax && body) {
                                                            var frame = doc.createElement('iframe'); // create
                                                                                                        // container
                                                            var frameData;
                                                            ...
                                                            // customize inputs
                                                            process(
                                                                    frameData.find('.' + baseClass),
                                                                    {}, true);

                                                            // extract HTML
                                                            frameData = frameData[0];
                                                            data = (frameData.body || frameData
                                                                    .getElementsByTagName('body')[0]).innerHTML;
                                                            frameData = null; // prevent memory
                                                                                // leaks
                                                        }
