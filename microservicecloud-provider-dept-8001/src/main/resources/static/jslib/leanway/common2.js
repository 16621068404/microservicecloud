...
com.leanway.checkSession = function() {
	$
			.ajax({
				type : "post",
				url : "../../../lnplatform/user",
				...
				success : function(text) {
					...
					if (tempData.code == "2") {
						...
						lwalert("tipModal", 2, "系统长时间未操作，登录失效，请重新登录",
								"forwardLogout()");
					}
				}
			});
...
function forwardLogout() {
	window.parent.location.href = "../../../lnplatform/user?method=logout&url=" + com.leanway.getHttpUrl();
}
	...
	try {
		...
		if (typeof (data) == "object") {
			...
			if (data.sessionStatus == "-1") {
				...
				lwalert("tipModal", 2, "登录超时，请重新登录！", 'com.leanway.logout()');
				flag = false;
			}
...
com.leanway.getHttpUrl = function ( )  {
	...
	var urlVal = allUrl.split("/lnui")[0]+"/platform";
	return urlVal;
}
...
com.leanway.dataTableClickMoreSelect = function(tableId, checkBoxName,
	...
	$('#' + tableId)
			.on(
					...
					'tbody tr',
					function(e) {
						/*for (var int = 0; int < tabmap.length; int++) {
							if (array[int] != null || onClick != undefined
									|| typeof (onClick) != "undefined") {
								array[int].clear();
							}
						}*/
						...
						var now = new Date();
						var thisValue = $(this)[0].cells[0].children[0].children[0].value;// 当前checkbox的值
 ...
 *
 *
 * @param type 1:当前页面的数据，2：跨页数据
 * @param tableName Map的key名称
 * @param checkBoxName datatables的checkbox名称
 */
...
com.leanway.getCodeMap = function (t, c, p, id, textVal) {
	...
	$.ajax({
		type : 'post',
		url : '../../../lnplatform/basicdata/codeMap',
		async : false,
		data : {
