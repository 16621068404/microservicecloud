$.IMSignalR = function (options) {
    ...
    $.connection.hub.qs = { "userId": options.userId,"userName":options.userName };
...
//    console.log(chat);
    ...
    chat.client.addMessage = function(name, message) {
         tencheer.dialogTop({ msg: name+":"+message, type: 'success' });
    };
    chat.client.ExitMessage = function() {
         tencheer.dialogTop({ msg: '提醒：您的帐号已在另一个地点登录，您将被迫退出系统！', type: 'error' });
         top.tencheer.login_status="login_exit";
//        tencheer.dialogMsg({ msg: '提醒：您的帐号已在另一个地点登录，您被迫下线！', type: 0,time:5000 });
//        tencheer.dialogConfirmMsg("注：您的帐号已在另一个地点登录，您被迫下线！", function (r) {
//            if (r) {
                //tencheer.Loading(true, "正在安全退出...");
...
IMInit = function (options) {
    ...
    var imMethod;
    var defaults = {
        userId: "<?php echo $userid;?>",//当前登录Id
        userName:"<?php echo $username;?>",                                   //当前用户名,
        toUserId: "",                                   //发送人Id
        toUserName: "",                                 //发送人
        windowId: "",   //当前窗口Id
...
//                console.log(item);
                ...
                if (userJson.length > 0) {
                    ...
                    $.each(userJson, function (j) {
                        var src = "statics/images/off-line.png"
                        if (userJson[j].F_UserOnLine == 1) {
                            src = "statics/images/on-line.png"
                        }
                        html += '<li>';
                        html += '<div class="message-oneface"><img src="'  + src + '" /></div>';
        ...
        updateUserStatus: function (userId, isOnline) {
            ...
            $message_wrap.find('.message-chatlist li').find('[data-value=' + userId + ']').prev('.message-oneface').find('img').attr('src', top.contentPath + '/statics/images/' + src + '');
    ...
    function UpDateLastUser(userLasrList) {
        ...
        $.each(userLasrList, function (i, item) {
            ...
            html += '<li>';
            html += '<div class="message-oneface"><img src="' + top.contentPath + 'statics/images/' + src + '" /></div>';
    ...
    function GetMessage(fromUer, content, dateTime,flag) {
        if (fromUer != options.userId) {
            if (options.windowId == fromUer)
            {
                ...
                html += '<img src="/statics/images/on-line.png" />';
...
IMIconflash = function (state) {
    ...
    if (state == 1) {
        if ($('body').find('embed').length == 0) {
          //  $('body').append('<embed src="statics/images/video/5103.wav">');
        }
        else {
            $('embed').remove();
            //$('body').append('<embed src="statics/images/video/5103.wav">');
...
IMRemove = function () {
    ...
    $obj.remove();
    $messagewrap.remove();
//    tencheer.dialogTop({ msg: '消息服务器连接不上', type: 'error' });
}