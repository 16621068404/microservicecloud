...
(function (factory) {
    ...
    function adjustDropMenuPos($input, $dropdownMenu, options) {
        ...
        if (options.autoDropup) {
            setTimeout(function() {
                ...
                if ( // 自动判断菜单向上展开
                    ($window.height() + $window.scrollTop() - $input.offset().top) < $dropdownMenu.height()+120 && // 假如向下会撑长页面
    ...
    function refreshDropMenu($input, data, options) {
        var $dropdownMenu = $input.parent().find('ul.dropdown-menu'),
            len, i, j, index = 0,
            tds,
            html = ['<table class="table table-condensed table-bordered table-sm" style="margin:0">'],
...
//            html.push('<thead bgcolor="#f5f6fa"><tr>');
            html.push('<thead bgcolor="#dceca9"><tr>');
            for (j in dataList[0]) {
    ...
    var defaultOptions = {
        ...
        listHoverStyle: 'background: #eaeda1; color:#000', // 提示框列表鼠标悬浮的样式
...
//                    console.warn('不是一个标准的 bootstrap 下拉式菜单或已初始化:', $input);
                ...
                $input.on('keydown', function(event) {
                    ...
                    if (~$.inArray(event.keyCode, [options.keyDown, options.keyUp,options.keyLeft, options.keyEnter])) {
                    ...
                    if ($input.attr("editable") == "false") return;
                    adjustDropMenuPos($input, $dropdownMenu, options);
                }).on('blur', function() {
