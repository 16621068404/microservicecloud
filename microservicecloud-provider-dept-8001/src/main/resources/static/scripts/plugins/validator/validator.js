...
数据验证完整性
**/
$.fn.Validform = function (errorObj=new Object()) {
    var Validatemsg = "";
        ...
        if (checkexpession != undefined) {
            ...
            switch (checkexpession) {
                case "NotNull":
                    {
                        if (isNotNull(value)) {
                            Validatemsg = "&nbsp&nbsp"+errormsg + " 不能为空！\n";
                            Validateflag = false;
                            $(this).focus();
                            $(this).click();
                            top.tencheer.dialogTop({ msg: Validatemsg, type: 'error' });
                            //tencheer.dialogMsg({ msg: Validatemsg, type: 2 });
                            errorObj.msg=Validatemsg;
                            return false;
//                            tencheer.dialogTop({ msg: Validatemsg, type: "error" });
//                            ValidationMessage($(this), Validatemsg); return false;
                ...
                case "LessThanZero":
                    {
                        if (isInteger(value) && parseFloat(value)<0) {
                            Validatemsg = errormsg + "不能小于零！\n";
                            Validateflag = false;
                            top.tencheer.dialogTop({ msg: Validatemsg, type: 'error' });
                            //tencheer.dialogMsg({ msg: Validatemsg, type: 2 });
                        }
                ...
                case "DoubleOrNull":
                    {
                        if (!isDoubleOrNull(value, $(this).attr("eqvalue"))) {
                            Validatemsg = errormsg + "必须为小数！\n";
                            Validateflag = false;
                            tencheer.dialogMsg({ msg: Validatemsg, type: 2 }); return false;
                           // ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
                    }
...
function ValidationMessage(obj, Validatemsg) {
    try {
        ...
        if (obj.hasClass('input-datepicker')) {
