			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '		<div class="upload_main single_main">';
                                        html += '			<div class="status_bar">';
                                       // html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
                                        html += '				<div id="tips" style="font-size: 15px;color:#e62a2a;" class="info">注：选择图片后需要点击上传图片按钮才能完成操作！</div>';
                                        html += '				<div class="btns">';
                                        html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
                                        html += '					<div class="webuploader_pick">选择图片</div>';
                                        html += '					<div class="upload_btn">上传图片</div>';
                                        html += '				</div>';
                                        html += '			</div>';
                                        ...
                                        html += '							<img class="upload_image" src="/statics/scripts/plugins/zyupload/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
                                        html += '						</div>';
                                        html += '					</a>';
                                        html += '				</div>';
					html += '			</div>';
...
//						console.info(files);
					...
					onSuccess: function(file, response) {
						...
						$("#uploadSuccess_" + file.index).show();
                                                $("#uploadImage_" + file.index).attr("src",data["dir"]);
                                                $("#uploadImage_" + file.index).attr("name",data["name"]);
//                                                console.info(file);
//                                                console.info("加入数组："+data["name"]+":"+data["dir"]);
//                                                image_array.push(data["name"]+":"+data["dir"]);
//                                                image_array[data["name"]]=data["dir"];
//                                                console.info(image_array);
