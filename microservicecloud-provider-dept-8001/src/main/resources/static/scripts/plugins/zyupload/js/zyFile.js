...
var ZYFILE = {
		...
		funAddImage:function(data){
                    var i=0;
                    for(var index in data){  
                        var item=data[index].split(':');
                            var _html='<div id="uploadedList_'+index+'" class="upload_append_list">';
                            _html+='<div class="file_bar">';
                            _html+='<div style="padding:5px;">';
                            _html+='<p class="file_name">'+item[0]+'</p>';
                            _html+='<span class="file_del" data-index="'+index+'" title="删除"></span>';
                            _html+='</div>';
                            _html+='</div>';
                            _html+='<a style="height:135px;width:180px;" href="#" class="imgBox">';
                            _html+='<div class="uploadImg" style="width:180px">';
                            _html+='<img id="uploadedImage_'+index+'" name="'+item[0]+'" class="upload_image" src="';
                            
                            var images=item[1].split('.');
                            if (images[1]!=="jpg" && images[1]!=="gif" && images[1]!=="bmp" && images[1]!=="png" && images[1]!=="ico")
                            {
                                _html+='/statics/images/filetype/'+images[1]+'.png';
                            }
                            else
                            {
                                _html+=item[1];
                            }
                            _html+='" style="width:expression(this.width > 180 ? 180px : this.width)">';
                            _html+='</div>';
                            _html+='</a>';
                            _html+='<p id="uploadedSuccess_'+index+'" class="file_success" style="display: block;"></p>';
                            _html+='</div>';
                           // $("#preview").prepend(_html);
                        ...
                        if($(".file_del").length>0){
                            var index= $(this).attr("data-index");
                                ...
                                $(".file_del").click(function() {
                                        $("#uploadedList_" + $(this).attr("data-index")).fadeOut();
//                                        image_array.delete(index);
