...
$(function() {
	...
	var ids;
	com.leanway.initSelect2("#mouldid", "../../../../"+ln_project+"/workCenter?method=queryMouldBySelect", "查询模具",true);
...
function search(){
	...
	if(mouldid != null && mouldid.length > 0){
		for(var i = 0 ; i < mouldid.length ; i++){
			mouldids += mouldid[i]+",";
		}
	}
	if(mouldids != "" && mouldids.length > 0){
		mouldidss = mouldids.substring(0,mouldids.length-1);
	}
	console.log("========="+mouldidss);
	...
	if(starttime==null||starttime==""||endtime==""||endtime==null){
		...
		return;
			 ...
			 $.ajax ( {
				...
				url : "../../../"+ln_project+"/balanceSheet",
				data : {
					"method" : "queryDispatchingOrderMouldByTime", 
					"starttime":starttime,
					"endtime":endtime,
					"mouldidss":mouldidss
						},
...
function viewChart(temp){
	
	
	var tempData = temp.date;  //  map(key,values)
	
	var tempHtml2 = "";
	//第一行
    tempHtml2 +='<tr>';
    tempHtml2 +='<th>模具名称</th>';
    tempHtml2 +='<th>产品编码</th>';
	tempHtml2 +='<th>产品名称</th>';
	tempHtml2 +='<th>派工单号</th>';
	tempHtml2 +='<th>计划数量</th>';
	tempHtml2 +='<th>开始时间</th>';
	tempHtml2 +='<th>结束时间</th></tr>';
	
		for(var key in tempData){
	
			if(tempData[key] == null || tempData[key].length == 0){
				continue;
			} else {
				
		    	for(var i = 0;i< tempData[key].length;i++){
		    		var productorname = tempData[key][i].productorname ? tempData[key][i].productorname:"";
		    		var productordesc = tempData[key][i].productordesc ? tempData[key][i].productordesc:"";
		    		var barcode = tempData[key][i].barcode ? tempData[key][i].barcode:"";
		    		var count = tempData[key][i].count ? tempData[key][i].count:"";
		    		var adjuststarttime = tempData[key][i].adjuststarttime ? tempData[key][i].adjuststarttime:"";
		    		var adjustendtime = tempData[key][i].adjustendtime ? tempData[key][i].adjustendtime:"";
		    		tempHtml2 +='<tr>';
		            tempHtml2 +='<td>'+key+'</td>';
		            tempHtml2 +='<td>'+productorname+'</td>';
		    		tempHtml2 +='<td>'+productordesc+'</td>';
		    		tempHtml2 +='<td>'+barcode+'</td>';
		    		tempHtml2 +='<td>'+count+'</td>';
...
//		            var timechild = endtimechild - starttimechild;
		...
		//            }
		           ...
		           // tempHtml2 +='<td colspan="2">'+ '<div style="height: 36px;white-space: nowrap;float: left;position:relative;z-index:999 ">' + adjuststarttime + "<br>" + adjustendtime +  '</div><div style="background-color: #7DB9DE;height: 36px;float: left"></div></div>' + '</td>';
