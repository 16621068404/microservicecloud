...
//
//		 $("#artificialrate2").html('<span title="作业人工时间系数">作业人工时间</span>');
//		 $("#leadtime2").html('<span title="本序与上序时间差">与上序时间差</span>');
//	 }
});
...
function queryleadtime(){
    var routeid = $("#routeid").val();
    $.ajax({
        type : "post",
        url : "../../../"+ln_project+"/processRoute",
        data : {
            "method" : "queryleadtime",
            "routeid":routeid
        },
        dataType : "json",
        async : false,
			...
			if(flag){
		           ...
		           if(tempData.status == "fail"){
		        	   ...
		        	   lwalert("tipModal", 1, tempData.info);
						...
						$("#saveOrUpdateProcessRouteId").attr({"disabled" : "disable"});
