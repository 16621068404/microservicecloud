...
var readOnlyObj = [{"id":"saveOrUpdateAId","type":"button"},{"id":"acquisition","type":"radio"},{"id":"serialnumber","type":"radio"},{"id":"batch","type":"radio"},{"id":"barcode","type":"radio"}];
...
$ ( function () {
	...
	//页面初始化只读
	com.leanway.formReadOnly("ProductorsForm");
...
var initTable = function() {
	...
	.DataTable(
			{
				"ajax" : "../../../../"+ln_project+"/productors?method=findAllProductorsByCenter",
				//	"iDisplayLength" : "10",
				'bPaginate' : true,
				"bDestory" : true,
				"bRetrieve" : true,
...
function queryProductorByName(){
	...
	oTable.ajax.url("../../../../"+ln_project+"/productors?method=findAllProductorsByCenter&searchValue=" + searchValue).load();
}
/**
 * 根据产品id 查找子公司
 * @author 熊必强
...
var addProductors = function() {
	...
	document.getElementById("headcompanyid").readonly=false;
				...
				"aoColumns" : [
				               {
				            	   ...
				            	   "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
				            		//   $(nTd).html("<div id='stopPropagation"+ iRow +"<input type='checkbox' name='checkSonCompany' value='" + sData + "'>");
				            		   $(nTd)
				            		   .html("<div id='stopPropagation" + iRow +"'>"
				            				   +"<input class='regular-checkbox' type='checkbox' id='"
				            				   + sData
...
var saveProductors = function() {
	...
	//去除两头空格
	barcodevalue = barcodevalue.replace(/^\s+|\s+$/g,"");
	...
	if($('#ProductorsForm').data('bootstrapValidator').isValid()){
		...
		$("#bodyproductorIssued").find('tr').each(function (index, temp) {
			...
			if ($(this).hasClass('row_selected')) {
				...
				//	var senderid=$(this)[0].cells[0].children[0].children[0].value;
...
//		}
		...
		$.ajax ( {
			type : "POST",
			url : "../../../../"+ln_project+"/productors?method="+ope,
			data : {
				"formData" : formData,
				"unitsData": unitsData
			},
...
//	}
	//getAttribute
			$("#unitConversionTable tbody:eq(0)").append("<tr> " +
					...
					"<td style='width:15%'> <select class='form-control' id='unitsid2"+i+"' name='unitsid2' ></select></td>"+//	"<td><input type = 'text' id='practicalcount"+i+"' name='practicalcount' class='form-control' /></td>" +
