...
$(function() {
	...
	$("input[id=daygantt]").click(function(){
		queryProcedureByProductDay();
	});

	$("input[id=monthgantt]").click(function(){
		queryProcedureByProductMonth();
	});
});
...
var searchStanderProcedure = function () {
	...
	oTable.ajax.url("../../../../"+ln_project+"/salesprogress?method=queryProducts&searchValue=" + searchVal+"&starttime="+starttime+"&endtime="+endtime).load();
}
			...
			.DataTable(
					{
						...
						"fnDrawCallback" : function(data) {
                            ...
                            com.leanway.dataTableClick("standerProcedureDataTable", "checkList", false,
                                    oTable, queryProcedureByProductMonth);
						}
 ...
 * 根据生产订单id查询相应的工序信息
 */
function queryProcedureByProductMonth(productionorderid) {
...
function getDayGantt(listdis,time,num){
	...
	//alert(start)
	enddates2.setHours(23);
	enddates2.setMinutes(59);
	enddates2.setSeconds(59);
		...
		if(weeklist!=null){
			for(var i=0;i<weeklist.length;i++){
				...
				month.push({
					"start":startdates,
					"end":enddates,
					"label": startdates.substr(0,10)+"日--"+enddates.substr(5,6)+"日"
				});
			}
     }
