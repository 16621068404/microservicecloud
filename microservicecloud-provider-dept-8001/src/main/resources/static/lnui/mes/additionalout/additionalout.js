	 ...
	 var confirmstatus =  $('input[name="confirmstatus"]:checked').val();
	    ...
	    if(data.length == 0) {
	        ...
	        if(confirmstatus != 2){
	            ...
	            if(details!=''){
	            	...
	            	for(var i in details){
	            		 ...
	            		 var mapid = details[i].mapid;
	  	            	 ...
	  	            	 var conversionrate = details[i].conversionrate;
	  	            	 var serialnumber=details[i].serialnumber;
	  	            	 var productorstatus=details[i].productorstatus;
	  	               
	  	            	if (mapid != null && mapid != "" && mapid != "null") {
	  	            		initSelect2("#mapid"+i,"../../../"+ln_project+"/additional?method=queryAllStorageMap", "搜索仓库");
	  	                    $("#mapid"+i).append(
	            ...
	            lwalert("tipModal", 1, "已出库的出入库单，不能再进行修改!");
	        }
	    }
...
function setTableValue(data,type){
   ...
   var tableBodyHtml = "";
	   ...
	   for(var i in data){
	       ...
	       tableBodyHtml += "  <td><input  type = 'text' id='count"+i+"' value="+data[i].count+" name='count' style='width: 110px;' onkeyup='isNumber()' onafterpaste='isNumber()' class='form-control' /></td>";
	       tableBodyHtml += "  <td> <input  type = 'text' disabled='false' id='conversionrate"+i+"' value='"+data[i].conversionrate+"' name='conversionrate' style='width: 80px;'  class='form-control'/></td>";
...
function addTable(){
	...
	if(ope =='addInOutAdditional'){//如果是新增杂入杂出，判断新增的状态
			...
			$("#inoutTable tbody:eq(0)").append("<tr> " +
					...
					"<td><select id='mapid"+i+"' name='mapid' style='width: 130px;'  class='form-control select2'></select></td>" +
					"<td><select id='productorid"+i+"' name='productorid' style='width: 130px;'  class='form-control select2'></select></td>" +
					"<td><input type = 'text' id='productordesc"+i+"' name='productordesc' style='width: 120px;'  class='form-control' readonly='true'/></td>"+
					"<td><select id='versionid"+i+"' name='versionid' style='width: 68px;'  class='form-control select2' readonly='true'></select></td>" +
					"<td><select id='batch"+i+"' name='batch' style='width: 130px;'  class='form-control select2'></select></td>" +
					"<td><input type = 'text' id='stockunitsname"+i+"' name='stockunitsname' style='width: 50px;'  class='form-control' readonly='true'/></td>"+
					"<td><input type = 'text' id='count"+i+"' name='count' style='width: 110px;' onkeyup='isNumber()' onafterpaste='isNumber()' class='form-control' /></td>" +
	...
	if(ope == 'updateAdditional'){//如果是修改杂入杂出,判断修改出入单的状态
			...
			$("#inoutTable tbody:eq(0)").append("<tr> " +
					...
					"<td><select id='mapid"+i+"' name='mapid' style='width: 130px;'  class='form-control select2'></select></td>" +
					"<td><select id='productorid"+i+"' name='productorid' style='width: 130px;'  class='form-control select2'></select></td>" +
					"<td><input type = 'text' id='productordesc"+i+"' name='productordesc' style='width: 130px;'  class='form-control' readonly='true'/></td>"+
					"<td><select id='versionid"+i+"' name='versionid' style='width: 68px;'  class='form-control select2'></select></td>" +
					"<td><select id='batch"+i+"' name='batch' style='width: 150px;'  class='form-control select2'></select></td>" +					
					"<td><input type = 'text' id='stockunitsname"+i+"' name='stockunitsname' style='width: 50px;'  class='form-control' readonly='true'/></td>"+
					"<td><input type = 'text' id='count"+i+"' name='count' onkeyup='isNumber()' style='width: 110px;' onafterpaste='isNumber()' class='form-control' /></td>" +
					"<td><input  id='conversionrate"+i+"'  name='conversionrate' style='width: 150px;'  class='form-control'/></td>" +
					"<td style='display:none'> <input class='form-control' type='hidden' id='stockunits"+i+"' name='stockunits'/></td>"+
	                "<td style='display:none'> <input class='form-control' type='hidden' id='mapid"+i+"' name='mapid' /></td>"+
					" </tr>");
		}
//	}
	...
	initSelect2("#batch"+i,"../../../"+ln_project+"/additional?method=queryBatchByProductorAndMap&mapid="+$("#mapid"+i+" option:selected").val()+"&productorid="+$("#productorid"+i+" option:selected").val()+"", "搜索批次号");
...
function deleteAdditional(type){
    ...
    if(data.length == 0) {
    	...
    	lwalert("tipModal", 1, "该订单已经出库库，不能删除！");
    	return;
    }else{
...
//	var inventoryid;
	...
	$.ajax({
	type : "post",
	url : "../../../"+ln_project+"/additional?method=delAdditionalOut",
...
function deleteAdditionalDetail(){
	...
	var ids = com.leanway.getCheckBoxData(1, "inoutTable", "boxid");     //明细id
	...
	if(arr.length == allIds){            //删除该表头所有的明细

		lwalert("tipModal", 2, "此操作会删除整个出入单,确定吗？"," sureDeleteAdditionalDetail(1,1)");
		return;
	}
	if(arr.length != allIds){             //删除该表头下选中所有的明细
...
function saveLabelData() {
	...
	if ($('#labelPrintDivForm').data('bootstrapValidator').isValid()) {
		    ...
		    var formData = formatFormJson(form);
		     ...
		     objArray.push(JSON.parse(formData));
		     console.info(objArray);
		     var printName = "其他出库标签";
		     var printFile = "qtckbq";
			
			com.leanway.sendReportData(printName, printFile, objArray);
