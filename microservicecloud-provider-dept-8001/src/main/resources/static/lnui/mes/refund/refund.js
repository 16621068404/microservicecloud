...
$ ( function ( ) {
	...
	refundDetailTable = refundDetailTable( );
	 ...
	 com.leanway.initSelect2("#deliveryid", "../../../../"+ln_project+"/deliveryOrder?method=queryDeliveryOrder", "采购入库单");
	 ...
	 $("#deliveryid" ).on( "select2:select", function ( e ) {
		 ...
		 refundDetailTable.ajax.url( '../../../../'+ln_project+'/refund?method=queryPurchaseRefundDetail&type=2&id=' + $(this).val( ) ).load();
	 });
	 ...
	 $("input[name=confirmstatusradio]").click( function( ) {
		...
		refundTable.ajax.url( '../../../../'+ln_project+'/refund?method=queryPurchaseRefundList&confirmstatus=' +  $(this).val( ) + "&searchValue=" + searchValue ).load();
	 });
...
var initRefundTable = function ( ) {
	...
	var table = $('#refundTable').DataTable({
		"ajax" : '../../../../'+ln_project+'/refund?method=queryPurchaseRefundList',
//		"iDisplayLength" : "8",
		'bPaginate' : true,
		"bDestory" : true,
		"bRetrieve" : true,
...
function setFormValue( data ) {
	...
	for ( var item in data ) {
	 ...
	 if (deliveryid != "" && deliveryid != null && deliveryid != "null") {
    	setValueToSelect2Func("#deliveryid", deliveryid, data.deliveryno);
	 }
}
...
//		},
		...
		{
			"mDataProp" : "purchasecount"
		},
		{
			"mDataProp" : "returncount"
		},
		{
			"mDataProp" : "calloutcount"
		}],
		"aoColumnDefs" : [ {
			"sDefaultContent": "",
			 "aTargets": [ "_all" ]
...
var editRefundDetailTable = function ( ) {
	...
	if (refundDetailTable.rows().data().length > 0) {
		...
		$("#refundDetailTable tbody tr").each( function( ) {
			...
			// 本单退货
			var calloutcount = refundDetailTable.rows().data()[index].calloutcount;

			if ( calloutcount == "null" || calloutcount == null) {
				calloutcount = "";
			}

			$(this).find("td:eq(5)").html('<input type="text" style="width:100px;" class="form-control input-sm" name="calloutcount" id="calloutcount" onblur="setDataTableValue(this, '+ index+ ',\'refundDetailTable\')"   value="'+ calloutcount + '">');
