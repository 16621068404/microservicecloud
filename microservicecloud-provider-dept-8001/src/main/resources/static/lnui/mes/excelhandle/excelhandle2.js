...
$('#uploadFileInput').on('fileuploaded', function(event, data, previewId, index) {
	...
	console.info(data.response.status)
	...
	resetFormFunc();
	 var tableBodyHtml = "";
	 if(data.response.importType==7){
		 if(data.response.bomResultMap!=undefined&&data.response.bomResultMap.errorinfo!=undefined&&data.response.bomResultMap.errorinfo[0] != null&&data.response.status=="success") {
			 ...
			 if(data.response.errorList != null&&data.response.errorList.length>0&&data.response.status=="success") {

					for(var key in data.response.errorList) {

						var length = data.response.errorList[key].length;

						tableBodyHtml+="<li>"+data.response.errorList[key].slice(0, length-2)+"<br>";
						tableBodyHtml+="<span style='font-size:16px;color:red;'>"+data.response.errorList[key][length-1]+"</span></li>"
		 ...
		 if(data.response.errorList != null&&data.response.errorList.length>0) {
...
$(function(){
	...
	initFileInput("#uploadFileInput", "../../../../"+ln_project+"/uploadFile?method=excelUpload");
});