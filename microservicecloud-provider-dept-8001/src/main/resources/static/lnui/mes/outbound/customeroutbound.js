...
var updatePurchaseOrder = function ( ) {
	...
	if (data.length == 0) {
		...
		lwalert("tipModal", 1, "请选择客户现场出库单修改!");
		return;

	} else if (data.length > 1) {

		lwalert("tipModal", 1, "请选择一条客户现场出库单修改!");
		return;

	} else {
		opeMethod = "updatePurchaseOrder";   // comment
		com.leanway.clearTableMapData( "purchaseOrderTable" );
		//开启可编辑状态
		com.leanway.removeReadOnly("purchaseOrderForm",readOnlyObj);
		//关闭可编辑状态  readonly(可以读)   disabled(不可以读)
		$("#tradetype").prop("readonly", true);
		$("#deptid").prop("readonly", true);
		$("#mapid").prop("readonly", true);
		$("#shortname").prop("readonly", true);
		$("#projectclass").prop("readonly", true);
		$("#compname").prop("readonly", true);
		$("#ladlenumber").prop("readonly", true);
		$("#permanentnumber").prop("readonly", true);
		$("#packageservice").prop("readonly", true);
...
var savePurchaseOrder = function ( ) {
	...
	if ($('#purchaseOrderForm').data('bootstrapValidator').isValid()) {
		...
		var checkboxs = $("#purchaseOrderTable :checked");
		var pgdids = [];
		for(var i = 0; i < checkboxs.length; i++) {
			pgdids.push(checkboxs[i].getAttribute('value'));
		}
		var additionalid = pgdids[0];
	    ...
	    $.ajax({
			...
			data : {
				"method" : "saveCustomerOutBoundOrder", //保存客户现场出库单
				"formData" : formData, 
			    "additionalStatus" : 1,
...
var initPurchaseProductorTable = function ( ) {
	var editTable = $("#purchaseProductorTable").DataTable({
			"ajax" : '../../../../'+ln_project+'/additional?method=queryOutBoundAdditionalDetail&conditions=' + encodeURI('{"additionalid": "1"}'),
			'bPaginate': false,
			"bRetrieve": true,
			"bFilter":false,
	        "scrollX": true,
			...
			"aoColumns" : [
			{
				"mDataProp" : "additionaldetailid",      //现场出库单明细
				"fnCreatedCell" : function(nTd, sData,
						oData, iRow, iCol) {
			...
			{
				"mDataProp" : "weight"  //重量  = 件数  * 转换率
			},
			{   
				"mDataProp" : "unitofmeasurement"  //计量单位，默认设置为   吨
					,"width": "60"
			},
			{
				"mDataProp" : "conversionrate"     //换算率
			},
			{
				"mDataProp" : "count"              //件数  （出库数量）   //在pc端进行输入出库数量  ---输入
			},
			{
				"mDataProp" : "stockunitsname"     //库存单位
					,"width": "60"
			},
			{
				"mDataProp" : "batch"              //批次
			...
			{
				"mDataProp" : "location"           //位置      //-----输入
					,"width": "134"
			},
			{
				"mDataProp" : "circularnumber"     //环数     //-----输入
			},
			{
				"mDataProp" : "repair"             //维修     //-----输入
					,"width": "134"
			},
            {
                "mDataProp" : "comment"     //备注
            },
            {
            	"mDataProp" : "schemenumber"     //方案号
             },
			{
				"mDataProp" : "outbounddate"       //出库日期
			},
			{
				"mDataProp" : "dateofcheck"        //审核日期
			} ],
			"aoColumnDefs" : [ {
				"sDefaultContent": "",
				 "aTargets": [ "_all" ]
...
var formatFormJson = function(formData) {
	...
	data = data.replace(reg, "");
    data += ",\"additionalDetailList\" : "+getDataTableData(purchaseProductorTable);
    //data += "\"updateSalesOrder\" : "+getSelectDataTableData(purchaseOrderTable)+"";
	data += "}";
	return data;
}
...
//				});
				...
				// 产品编码  【存货编码】
				var productorname = purchaseProductorTable.rows().data()[index].productorname;
				$(this).find("td:eq(1)").html('<input type="text" class="form-control" style="width: 100px" name="productorname" id="productorname" readonly value="'+ productorname + '">');
				// 产品名称 【存货名称】
